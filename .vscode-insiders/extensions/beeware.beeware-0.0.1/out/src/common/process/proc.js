...
const stdErr_1 = require("../errors/stdErr");
...
class ProcessService {
    ...
    execObservable(file, args, options = {}) {
        ...
        const output = new rxjs_1.Observable(subscriber => {
            const disposables = [];
            const on = (ee, name, fn) => {
                ee.on(name, fn);
            ...
            const sendOutput = (source, data) => {
                ...
                if (source === 'stderr' && options.throwOnStdErr) {
                    subscriber.error(new stdErr_1.StdErrError(out));
                }
                else {
    ...
    exec(file, args, options = {}) {
        ...
        proc.once('close', () => {
            ...
            if (stderr && stderr.length > 0 && options.throwOnStdErr) {
                deferred.reject(new stdErr_1.StdErrError(stderr));
            }
            else {
