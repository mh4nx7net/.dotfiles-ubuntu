...
function renderMasonryRepoStatus(repoStatusMap, check = true, upload = true, stage = false, amend) {
    let dataline = ``;
    const maxHeight = 4; // line
    const maxWidth = 19; // char
    repoStatusMap.forEach((repoStatus, project) => {
        let cls = "", prjCmd = encodeURI('command:repotool-cmd:openproject?' + JSON.stringify([`${cmnUtil.getProjWorkspacePath(project)}`])), prjCls = "", refresh = "", rfsCLs = "", fileCmd = "", filesHtml = "", height = 2, // at least project & branch;
...
exports.renderMasonryRepoStatus = renderMasonryRepoStatus;
class RepoStatusContentProvider extends baseView.BaseContentProvider {
    constructor(context) {
        super();
...
exports.RepoStatusContentProvider = RepoStatusContentProvider;
//# sourceMappingURL=repoStatusView.js.map