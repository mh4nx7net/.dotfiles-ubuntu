...
const repoUploadView = require("../comp/repoUploadView");
...
let provider = null;
let pageId = "";
const commitMsg = `[Detail]

[Solution]

`;
function checkFinish(mountPath, allDone) {
    status_1.updateSingleRepoStatus(mountPath);
    if (allDone.some((x) => { return x == null; })) {
...
function validateInput(proj, uploadingRepos) {
    if (!uploadingRepos.hasOwnProperty(proj)) {
        cmnUtil.showErrorMessage(`Project mismatched: \n
Selected: ${JSON.stringify(cmnUtil.getObjectKey(provider.selectedProjInfo))} <---> Uploaded: ${JSON.stringify(cmnUtil.getObjectKey(uploadingRepos))}`, `Project mismatched! Upload projects are different from selected projects!`);
        return false;
    }
...
function repoUpload(uploadStr) {
    try {
        let buf = urlUtil.decodeBase64(uploadStr);
        let uploadingRepos = JSON.parse(buf.toString());
...
exports.repoUpload = repoUpload;
function amendCommit(proj, isAmendToggled) {
    ...
    if (!isAmendToggled) {
        try {
            ...
            cli.getInstance().doGitLog(mountPath, `-1 --format="%s%n%n%b"`).then((msgs) => {
